...
{
  Serial.print("\n");
  Serial.print("?  - Prints this help menue\n");
  Serial.print("R  - Read DATA\n");
  Serial.print("W  - Writes DATA\n");
  Serial.print("Z  - DATA - test purpses\n");
  Serial.print("K  - Erase DATA on chip\n");
  Serial.print("Lx - x=1 set LOCK-BIT1 ** x=2 set LOCK-BIT2\n");
...
// 1.2mS during normal programing
//10.0mS during erasing the MEMEOR
void AT89c2051PROG_toggle(unsigned int mSec)
{
...
void SetAT89c2051Modes(unsigned char ModeVal)
{
  ...
  switch (ModeVal)
  {
    ...
    case 3:
      //          RST   P3.2    P3.3  P3.4  P3.5  P3.7
      //Erase     12V   H-L-H   H     L     L     L
      PORTB |= BIT2;  //12V on RST
      PORTB |= BIT3;  //RST-LOGIC-HIGH
      PORTC |= BIT3;  //P3.3 L
      PORTB &= ~BIT5;  //P3.4 L (enable)
      PORTC &= ~BIT2;  //P3.5 L
      PORTC &= ~BIT1;  //P3.7 L
      //PORTB |= BIT4;  //P3.2 TOGGLE
      AT89c2051PROG_toggle(10);
      Serial.println("***CHIP MEMORY has been erased***");
      SetAT89c2051Modes(2);
      break;
    case 4:
      //          RST   P3.2    P3.3  P3.4  P3.5  P3.7
...
void AT89cPREPBurnDATA()
{
  ...
  //
  PORTB &= ~BIT2;    //NO 12V on RST
  PORTC &= ~BIT4;    //XTal1 = LOW
  PORTB &= ~BIT3;    //RST-LOGIC-LOW
  //Turn VCC power off
}
...
void AT89cEraseDATAonChip()
{
  SetAT89c2051Modes(3);
}

void AT89cSetLockBits()
{
  unsigned char wichBit=' ';
  if (Serial.available() > 0) wichBit = Serial.read();
  if(wichBit=='1')
    SetAT89c2051Modes(4);   //READ-ONLY
  else if(wichBit=='2')
    SetAT89c2051Modes(5);   //FULL PROTECTED
  else Serial.println("\n***Unkown BIT***");
}

//char AT89CXXchip=2;               //2 = 89x2051
void AT89cSetICmodel()
{
  unsigned char wichBit=' ';
  if (Serial.available() > 0) wichBit = Serial.read();
  if(wichBit=='1')
  {
    AT89CXXchip=1;
    SetAT89c2051Modes(2);   //READ SIGNATURE
  }
  else if(wichBit=='2')
  {
    AT89CXXchip=2;
